...
Assert-False (Test-IsNewCommentLine -Content '### `slug` <!-- x -->' -FilePath '.github/pr-quality-gate/HIGH-TIER-SLUGS.md') 'HIGH-TIER-SLUGS.md is path-excluded (generated projection of the catalog)'
Assert-True (Test-IsNewCommentLine -Content '<!-- a real comment -->' -FilePath 'docs/HIGH-TIER-SLUGS.md') 'a like-named HIGH-TIER-SLUGS.md outside the gate path is NOT excluded'
Assert-True (Test-IsNewCommentLine -Content '<!-- a real comment -->' -FilePath 'nested/.github/pr-quality-gate/HIGH-TIER-SLUGS.md') 'a nested decoy HIGH-TIER-SLUGS.md is NOT excluded (anchor is repo-root-relative)'
