...
function New-Block {
    ...
    if (-not $NoGate) {
        $lines += "gate|diff_shown=yes:t3|diff_approved=$DiffApproved|staged_diff_verified=$StagedVerified|profile=full|author_identity=Jane <j@x.io>|commit_ownership=$Ownership|rule_coverage_passed=$RuleCoverage|pr_creation=deferred"
    }
    if (-not $NoSubjectLine) {
        $lines += "subject|proposed_subject=`"$ProposedSubject`"|subject_approved=$SubjectApproved|format_check=single_line:yes"
    }
