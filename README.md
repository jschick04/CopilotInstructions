...
3. **`scripts/check-comment-audit.ps1` + `scripts/check-playbook-refs.ps1` + `scripts/lib/comment-audit-helpers.psm1` + `scripts/lib/repo-root.psm1` + `scripts/tests/check-comment-audit.tests.ps1` + `scripts/tests/repo-root.tests.ps1`** - copy all six artifacts from this instructions repo. The CLI wrapper `check-comment-audit.ps1` imports modules from `lib/comment-audit-helpers.psm1` and `lib/repo-root.psm1`; without the module file, the CLI exits `INVOCATION_FAILED` immediately. The workflow runs both listed test files unconditionally - without them, CI fails. The scripts have no instructions-repo-specific hardcoded paths; they work from any project root that has `.git`, a `.github/playbooks/` folder (for the ref-check job), and the `.github/pr-quality-gate/audits/last.md` convention. If you do NOT want the `script-tests` CI job, delete that job stanza from the copied workflow.
...
- **Model registry decoupling** (`.github/playbooks/multi-model-review/current-model-registry.md`) - capability tier → current model name mapping for multi-model-review panel slots. Playbooks reference abstract tiers (`heavy-claude-xhigh`, `heavy-gpt-premium`, `heavy-gemini-premium`, `light-claude`, etc.) instead of literal model names, so model deprecations / renames update in ONE place. Representative current mappings: `heavy-claude-xhigh` = `claude-opus-4.7-xhigh`; `heavy-gpt-premium` = `gpt-5.5`; `heavy-gemini-premium` = `gemini-3.1-pro-preview`; `light-gpt` = `gpt-5.4-mini`. See the registry for the full 12-tier table + runtime-fallback / substitution rules.
