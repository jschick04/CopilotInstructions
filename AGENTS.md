...
1. Ensure `pre-pr-creation-review.md` (§2D) has emitted a `PRE-PR REVIEW COVERAGE` block in the conversation with `pr-creation-status: READY-pending-user-approval` per §2D G3. Absent → STOP and run §2D first; the gh pr create tool is forbidden per §2D G6.
2. Present the PR title, body, and target branch to the user.
3. Call `ask_user` with the full details and options to approve, edit, or reject.
4. Re-emit the `PRE-PR REVIEW COVERAGE` block in the same turn as the `gh pr create` invocation with `pr-creation-status: READY-re-emitted-after-user-approval` per §2D G3 (with same-state re-check — git HEAD/base unchanged since initial emission).
5. Only execute `gh pr create` after step 4's re-emitted block AND the user's explicit approval from step 3.
...
| Strong GitHub-to-ADO replication trigger — user is asking to **batch-replicate GitHub issues into ADO** (verb-led: *"replicate GitHub issues to ADO"*, *"sync GitHub issues to ADO"*, *"create ADO items from GitHub issues"*, *"import GitHub issues into ADO"*; or noun-led: *"replicate issues to deliverables"*, *"create deliverables from GitHub issues"*, *"create bugs from GitHub issues"*). The discriminator is the **source**: if the user mentions a GitHub repo or issues as the source for ADO items, route here — not to `ado-task-planning.md`. | OFFER `.github/playbooks/github-to-ado-replication.md` (always-confirm via `ask_user`) |
...
| Strong cross-file-bug-investigation trigger — user is asking for a **panel-driven bug hunt on UNCHANGED cross-file code** (verb-led: *"find bugs in"*, *"trace the bug across"*, *"hunt for bugs in"*, *"hunt for race conditions in"*, *"hunt for memory leaks in"*, *"look for race conditions in"*, *"look for state-mutation bugs in"*, *"investigate the interaction between"*; or cross-file-qualified verb-led: *"review the interaction between"*, *"audit the interaction between"*, *"review the flow between"*, *"audit the flow between"*; or noun-led: *"bug hunt across"*, *"bug-finding pass on"*, *"cross-file investigation of"*, *"interaction review of"*, *"interaction analysis of"*). Bare unpaired *"review"* / *"audit"* / *"trace this through the code"* fall into the ambiguous artifact-adjacent category and trigger one short clarifying `ask_user`. Discriminator vs `codebase-architecture-audit` (ranked DEBT list), `least-privilege-audit` (6-axis visibility), `code-review` sub-agent (DIFF review of recent changes), and `system-framing` (orientation map) is MECHANISM-led (panel + symptom-selected lanes) — see playbook Purpose section for the full 6-line canonical discriminator block. | OFFER `.github/playbooks/cross-file-bug-investigation.md` (always-confirm via `ask_user`) |
...
| About to add ANY new comment OR substantively rewrite an existing one (any language, any file) | AUTO-FIRE `.github/playbooks/comment-protocol.md` (sub-step) |
...
- **Panel binds to a specific artifact (HARD GATE).** A panel only certifies the exact artifact reviewed. Sub-decision panels (single library placement, single naming choice) do NOT satisfy a plan-level gate. If the plan is revised, scope-expanded, or has parts added after panel convergence, a new panel MUST run on the changed artifact. Before any implementation tool call, the agent emits a literal `PANEL CONVERGED` certification block (with artifact-hash, byte count, revision marker, round number, and verdicts). Full procedure in `.github/playbooks/review-workflow-gates.md` §1A.
- **Hard-stop tool list (HARD GATE).** Until a current artifact-binding certification is present in the conversation, the agent MUST NOT call: `create`, `edit`, `powershell`/`bash` with file-write or file-move or project-scaffold operations, `git add`, or sub-agent launches with implementation intent. Read tools (`view`, `grep`, `glob`, read-only shell, `ask_user`, `sql`, review/research sub-agents) remain available. Plan-file edits BEFORE panel are allowed; plan-file edits AFTER panel invalidate certification. **`exit_plan_mode` approval / plan-summary `ask_user` approval / runtime "proceed with implementation" messages are NOT certifications** — they are scope approval; the panel pass is the independent second condition that MUST also be satisfied. **Instruction-repo edits** (`.github/playbooks/**`, `.github/instructions/**`, `AGENTS.md`, `copilot-instructions.md`, `manifest.yaml`) are §1B tool calls — no "meta-work" / "small tweak" exemption; the panel-reviewing-instruction-changes prompt MUST include explicit focus on self-consistency, escape-hatch analysis, enforcement mechanism, model/path/tool-name stability, and project-agnosticism. Full enumeration + carve-outs + rationalization-defeat subsections in `.github/playbooks/review-workflow-gates.md` §1B.
- **Pre-PR-creation multi-model review (HARD GATE — §2D).** Before any PR-creation or review-visibility transition tool call (`gh pr create`, `gh pr ready`, draft toggles, `glab mr create`, `tea pr create`, `az repos pr create`, equivalents on any forge, plus MCP-server tools with PR-creation intent — full enumeration in `pre-pr-creation-review.md` G6), a multi-model heavy panel (≥4 reviewers per slate floor) MUST run on the FULL branch diff (`<base>..HEAD`) with the 11-category Copilot-mirror prompt template (`multi-model-review/pr-creation-mirror-prompt.md`). Every reviewer-flagged `blocking` finding MUST be resolved via `fixed` / `dismissed-source-grounded` / `routed-deferred-with-tracker-and-ask_user` (G4 conditions) — no other resolution path is valid for blocking. A `PRE-PR REVIEW COVERAGE` block (initial emission + re-emission after user-approval) MUST appear in the same turn as the PR-creation tool call. Strict mandatory — panel run, must-fix=0, block emission, disposition per finding, and forbidden-tool list are NOT user-waivable; convergence model and slate composition ARE user-waivable within floors. Full procedure + waive matrix + bootstrap exemption (G7, narrow scope — introducing PRs of NEW gates only, not modifications) in `.github/playbooks/pre-pr-creation-review.md`. Invoked from `pre-pr-push.md` Step 5 (single source of truth for when this gate fires).
- **Per-rule acknowledgement (HARD GATE — §1A.1).** Every panel verdict AND every pre-commit ledger MUST emit a `core_rules_acknowledged` block enumerating every HIGH-tier review-pass-only catalog slug listed in `.github/pr-quality-gate/HIGH-TIER-SLUGS.md` (generated by `scripts/sync-critical-rules.ps1` from `pattern-catalog.md`). Each slug requires `status: applied | not-applicable` plus per-site `file:line:disposition` citations (for `applied`) or a ≤30-word `rationale` (for `not-applicable`). Aggregate counts alone are INVALID — per-site enumeration is mandatory. The runner cross-references rg-violation counts against acknowledged sites; divergence BLOCKS unless `divergence_acknowledged: <≤50-word specific reason>` is added (tautology meta-rule: reason MUST cite a SPEC, BCL quirk, or external constraint by name, not restate the rule text). Canonical schema in `.github/pr-quality-gate/panel-policy.md` §Per-rule acknowledgement.
- **Anti-recidivism enforcement (HARD GATE — §1A.2).** For PRs with prior entries in `.github/pr-quality-gate/data/panel-misses.csv` (matched by opaque `pr_ref` token like `consuming-pr-8`), invoke `invoke-panel.ps1 -PrRef <ref>` (panel-launch path) OR `gate-runner.ps1 -PrRef <ref>` (gate-check path) to emit an anti-recidivism preamble listing every prior slug hit on the PR. Reviewers and the pre-commit ledger MUST include `verified-no-recurrence: <slug>` per prior slug with `fix_evidence` (`commit_sha` OR pasted `diff_hunk`) demonstrating the prior occurrence is resolved. Textual ack without fix_evidence is treated as NEEDS_REWORK.
- **Full-scan-against-new-rule enforcement (HARD GATE — §1A.3).** Every catalog edit (new slug OR enhanced audit-method on an existing slug) triggered by a PR-review bot finding REQUIRES the agent to IMMEDIATELY run the new/enhanced rule's `Audit method` clause against the FULL PR diff (not just the bot-flagged site) and report `full_scan_results: { <new_or_enhanced_slug>: { sites_scanned: <int>, gaps_found: <list of file:line>, gaps_fixed_in_this_amend: <list>, gaps_deferred_with_reason: <list> } }` in the `PRE-COMMIT GATE PASSED` block. Without this scan, the same PR can carry MULTIPLE instances of the new pattern that surface one-by-one in future bot rounds (consuming-pr-8 had cancellation-handling findings in 5 separate rounds before the missing-loop-CT-check rule was added in round 16 — earlier full-scans would have collapsed the iteration). Catalog slug `full-scan-against-new-rule-not-triggered-after-bot-finding` enforces this at review time.
...
- **Pre-commit hygiene cleanup runs whole-solution, not just touched files.** Moves and rename refactors leave stale `using`s and over-qualified type references in *consumer* files that the diff does not list. Restrict the cleanup to the using / qualifier hygiene diagnostics so it does not churn unrelated style (collection initializers, expression preferences, etc.). Touched-file imports / usings sorted and unused removed. **For any move / rename / namespace change, re-grep ALL consumer categories across the WHOLE solution, not just the declaring project:** production `.cs`, **test-project `.cs` (the most-missed category)**, framework markup and its code-behind (`.razor` / `.razor.cs` / `_Imports.razor`, and `.xaml` `clr-namespace` / `xmlns`), and project files (`.csproj` `<ProjectReference>` / `<Compile Include>`). A consumer that resolved the type via enclosing-namespace scoping needs an explicit `using` once the type moves to a non-enclosing namespace. Canonical using order when adding one: namespace usings sorted first, then `using static`, then aliases LAST. The full-solution build INCLUDING test projects is the safety net, but the enumeration is the source of truth, so a red build is a process miss, not the detector.
...
- **Touched-file vertical-slice (VSA) audit applied.** Trigger: the diff ADDS a new type or file, MOVES or RENAMES a file, adds a root-level file to an assembly that already has feature folders, or introduces a file containing more than one top-level type. Each such file must land in the correct slice (feature / domain folder) or, for a genuine cross-slice / cross-asm type, in `Common/<Domain>/` (never flat `Common/`, never a KIND-bucket like `Models/` / `Helpers/` / `Utils/`); one top-level type per file (split multi-type files); no root-level outlier in an assembly that otherwise organizes by folder. A uniformly-flat small assembly with NO folders is NOT an outlier and must NOT be folderized just to add structure (that is gold-plating per §3.13); fresh grep beats any cached survey or prior audit verdict. Procedure: the vertical-slice lens of `codebase-architecture-audit.md` at touched-file scope. Recorded as the `vsa-audit` ledger row.
...
- **Prior-PR-review sweep (HARD GATE).** Every commit-bound change runs a two-scope sweep: (A) the current branch's PR thread (if any) for review comments already addressed there, and (B) the last 10 merged PRs in the repo for review patterns the team has already learned about (both bot and human reviewers). Pattern-extract each comment (the class of issue, not the specific file/line) and sweep the current diff for matches. Agent MUST output a one-line-per-pattern report (`Prior-PR-review sweep: ran, M patterns checked, N findings. - <pattern from PR #X>: matches/no matches`) before showing the diff. Skip allowed only when the repo has no prior merged PRs OR when the change has no production-code edits — those facts documented explicitly. Full procedure in `.github/playbooks/review-workflow-gates.md` §2A.
- **DRY remediation gate (HARD GATE).** Run a DRY audit on the staged diff. Refactor any duplication where 2+ files share ≥5 lines of substantively-identical logic, 3+ instances of the same pattern appear, or a new file is a parameterized copy of an existing file. Default action is refactor (base class / helper / generic / extension / partial / component-inheritance). If refactoring is genuinely premature or inappropriate, present the duplication to the user via `ask_user` for a refactor-or-waive choice. The `dry-audit` ledger row in §2B records the outcome with `ran, N duplications, K refactored, J waived` and per-finding decisions. Full procedure + waiver semantics in `.github/playbooks/review-workflow-gates.md` §2C.
...
- **Post-code-change ledger emitted (HARD GATE).** Before any `git add` against staged-for-commit files, the agent MUST emit a literal `POST-CODE-CHANGE LEDGER` block in the **current turn**, enumerating the status of every post-code-change gate that applies to the staged content (hygiene-cleanup, touched-file-LPA, vsa-audit, recurring-pattern-sweep, prior-PR-review-sweep, post-code-change-panel, comment-audit-§3.1, emdash-scan, build, tests, diff-shown, commit-message-approved). Each gate row is `ran`, `N/A: <reason>`, or `user-waived: "<quote from current turn>"`. Waivers from earlier turns do NOT carry forward; "the user said staged-means-reviewed on commit N" cannot waive any gate on commit N+1. Without the ledger fully populated and present in the current turn, `git add` is forbidden. Full format + waiver semantics + skip conditions in `.github/playbooks/review-workflow-gates.md` §2B.
...
- Diff shown to user; explicit approval received. **Applies to ALL commit-producing operations without exception** — fresh commits, `--amend`, fixups, cherry-picks, rebases. No `git add` or `git commit` (including `git commit --amend`) until the user has seen the diff and approved. **There is no "quick fix" or "trivial amend" exemption.** PR review fixes, typo corrections, rename sweeps, and any other change — no matter how small or obvious — require the diff to be shown and approved before the commit/amend executes. The rationale "the user already approved the original commit" does not carry forward to amends that change its content.
- **`POST-CODE-CHANGE LEDGER` block emitted before `git add` (HARD GATE).** Mirrors §1A's `PANEL CONVERGED` enforcement at the commit boundary. The ledger enumerates every post-code-change gate's status (ran / `N/A: <reason>` / `user-waived: "<current-turn quote>"`) and MUST appear in the same turn as the `git add`. Previous-turn waivers do not carry forward; a fresh ledger is required for every commit (including amends, fixups, post-merge resolutions that lead to a commit). Full format in `.github/playbooks/review-workflow-gates.md` §2B.
- **HARD STOP before every `git add`.** Before executing ANY `git add` command, the agent MUST have ALREADY called `ask_user` in the current turn with the diff output and received explicit approval. If `ask_user` has not been called and approved in this turn, DO NOT execute `git add`. This is a sequencing constraint, not a judgment call — the `ask_user` approval MUST precede the `git add` in the tool-call sequence. Batching `git add && git commit` into a single shell command without prior `ask_user` approval is a violation. The correct sequence is always: (1) `git diff` to show changes, (2) `ask_user` with diff summary, (3) on approval: `git add` + `git commit` in a subsequent tool call.
- **Panel READY does NOT substitute for diff-approval on project repos.** A panel `READY` verdict (pre-implementation, pre-PR-creation, or any other slot) is technical review; the diff-approval gate above is user review. Both are independent on project repos; both must pass. Chaining panel READY into commit-producing tool calls, `git push`, or `gh pr create` without an intervening `ask_user`-based diff-approval step is a process violation. This rule does not modify the existing `.github/playbooks/review-workflow-gates.md` §1B project-vs-instruction-repo asymmetry. Full rule in `.github/pr-quality-gate/panel-policy.md` §"User diff-approval after panel READY".
...
- **Branch-wide vertical-slice (VSA) audit run** when `git diff <base>..HEAD` adds / moves / renames any type or file, adds a root-level file to a folder-organized assembly, adds a new top-level type to an existing file, or introduces a multi-type file. Same checks as the touched-file VSA gate, re-grepped against the FINAL branch state (small per-commit placements can together leave a type-bucket or a misplaced root file that no single commit revealed). Skipped only when the branch has no such structural delta, that fact recorded. Procedure: vertical-slice lens of `codebase-architecture-audit.md` at branch-wide scope.
- **Branch-wide prior-PR-review sweep run.** Two-scope sweep against the FULL branch diff (`git diff <base>..HEAD`), not just the latest commit: (A) the current branch's PR thread (if any) and (B) the last 10 merged PRs in the repo (bot + human comments). Catches "patterns the team has already flagged" that may have been re-introduced in branch commits since the last per-commit sweep. Same one-line-per-pattern report format as the post-code-change gate. Mandatory before opening / re-pushing any PR; silent skip is the failure mode this gate prevents. Full procedure in `.github/playbooks/review-workflow-gates.md` §2A.
...
- State read-back output per `pre-pr-push.md` before claiming ready (zero-count carve-out) — includes the 11-field state predicate (incl. `pushCredentialsVerified` + `preCreationReviewStatus`) plus `sandboxPriorExposureConfirmation` informational field.
- No "ready to push" claim until push credentials verified (§4.2), per-commit audit, branch-wide sweep, AND branch-wide least-privilege audit plus branch-wide vertical-slice (VSA) audit (each when applicable) are done OR user explicitly skipped (with recorded warning).
...
- **PR review comments are hard blockers.** Every comment must be root-cause analyzed, similar patterns swept across the diff, and instructions updated if a gap is revealed. Repeat findings across review rounds are a process failure. Full procedure in `.github/playbooks/review-workflow-gates.md` §2.
...
- Friend-grant proliferation is a coupling cost across ALL languages — adding a NEW friend-grant (C# IVT, Java `module-info` export, Kotlin friend-path, TS `package.json` exports entry, etc.) exposes ALL current AND future internals of the granting asm/module to the receiving one. Default precedence when faced with KEEP-PUBLIC vs ADD-NEW-FRIEND-GRANT: (1) split-member visibility (public type, internal-only members where possible), (2) co-locate the type with its consumer (when there is a single consumer and co-location respects slice ownership), (3) DI-seam extraction (public interface + public DI registration extension + internal sealed impl, for multi-consumer or DI-amenable cases where co-location is domain-wrong), (4) keep public, (5) add new friend-grant (LAST resort). Re-using an existing friend-grant is free; adding a new one is not. Full ladder + per-language techniques in the playbook.
...
- **Scope reduction requires explicit user sign-off.** The agent does NOT have authority to unilaterally drop scope. All deferrals/drops presented via `ask_user` with item, rationale, and dissenting opinions. Full procedure in `.github/playbooks/review-workflow-gates.md` §3.
...
- **Sub-agent model selection defaults.** Override the runtime default model on every sub-agent launch to maximize reasoning depth. Models are named by capability tier; resolve tier → current model id via `multi-model-review/current-model-registry.md`:
  - **Rubber-duck** passes: `heavy-claude-standard` tier.
  - **Standalone code-review**: `heavy-claude-xhigh` tier.
  - **Explore** agents for architecture, design-spec grounding, or multi-module cross-cutting investigation: `light-claude-balanced` tier. Simple single-file lookups may use the default.
  - **General-purpose** sub-agents doing reasoning-heavy work (debugging, synthesis, migration planning, ambiguity resolution): `heavy-claude-standard` tier.
  - **Security-review**: `heavy-claude-xhigh` tier.
  - **Multi-model review panels**: exact per-slot tiers defined in `multi-model-review/intake.md` item 4 (canonical source).
...
- **🚨 Three-step comment protocol — HARD GATE on every NEW or substantively-rewritten comment line in the diff.** Keyed OBJECTIVELY on what ends up in the diff — NOT on the agent's subjective sense of "needing" a comment. **Step 1 = clarity check** (is the surrounding code + every relevant function / method / parameter / variable / type / field / property name already clear? → no comment); **Step 2 = rename check** (can a better name or small helper extraction carry the fact? → rename + drop); **Step 3 = comment-approval gate** (call `ask_user` with proposed text + code snippet + reason + allowed-case BEFORE the `edit` that introduces the comment; on reject OR no response within the runtime's prompt window → DROP). **Headless / `ask_user` unavailable → DROP the comment, NEVER block the change** (a comment is never load-bearing enough to deadlock a commit; "headless" is a concrete predicate defined in the playbook — agent uncertainty about user availability is NOT headless). **Categorical bans vs default-OFF:** a TRULY CATEGORICAL ban (e.g., `csharp.instructions.md` no XML doc on `private` members) is NOT reachable via step 3 — the answer is *no comment*, you never ask; DEFAULT-OFF rules (e.g., new public/internal API XML doc default-OFF per `csharp.instructions.md`) ARE reachable via step 3 but the bar is HIGH — only propose when the language-file's narrow allowed scenarios genuinely apply, cite the scenario in the justification. **Sub-agents (general-purpose, task, research, code-review, etc.) must NEVER introduce comments via `edit` calls** — they may propose comments in return-value text only; the orchestrator runs steps 1–3 on integration and deletes any sub-agent-emitted comment that bypasses step-3 approval before showing the diff. The protocol governs every comment syntax in every language. **Canonical exempt categories** (these 6 machine tokens are the ONLY valid `approval_turn: n/a — exempt:` values — the ledger fails closed on any other label, no inventing new categories at audit time): `typo` | `deletion` | `stale-comment-fix-per-§3.9/§3.10` | `generated` | `vendored` | `THROWAWAY-header`. Full procedure (concrete headless predicate, AFK / no-response handling, batch cap, exempt-category precise definitions, persisted audit-file convention, sub-agent integration audit) in `.github/playbooks/comment-protocol.md`.
- **Hard prohibitions** (do not commit any of these — no exceptions):
  ...
  - **No panel-artifact references.** Comments must not mention `Slot N`, `Round N`, `PR N+M`, pre-implementation panel phase names, "the bot caught this", "the bot's finding", or any other review/planning artifact. Comments describe what the code IS, not the review history that shaped it. Catalog enforcement: `panel-artifact-leakage` (extended regex catches `\bbot\b.*\b(caught|finding|flagged|round)\b`).
  - **No test section-separator banners.** `// --- BuildRange ---`, `// --- Resolve ---`, `// region: setup`, and similar block labels are navigation cruft, not load-bearing; the test method names already group by behavior. Delete them.
  - **No comments that restate a test's name or scenario.** A test named `Resolve_ReverseShiftRange_IsSymmetric` does not need `// reverse range yields the same ordered set`; a `Method_Scenario_Expected` name already encodes the case. Keep a test comment ONLY when it explains a non-obvious test artifact the name cannot (for example, a mock that freezes intermediate state so the asserted value looks counterintuitive).
...
- **Mandatory self-review pass before showing diff:** enumerate every NEW or substantively-rewritten comment line in the diff. For each one, confirm the bullet carries an `approval_turn:` citation in one of these forms: **(i)** a step-3 `ask_user` approval turn / message ref PLUS an `allowed-case` matching one of the 3 *Allowed* cases above ("non-obvious invariant" / "external constraint" / "trade-off"); **(ii)** `n/a — exempt: <category>` where `<category>` is one of the 6 canonical tokens (`typo` | `deletion` | `stale-comment-fix-per-§3.9/§3.10` | `generated` | `vendored` | `THROWAWAY-header`) — allowed-case NOT required for exempt entries; **(iii)** `n/a — degraded-mode-drop` (headless context, the comment was dropped before reaching the diff); **(iv)** `n/a — no-response-drop` (ask_user available but user didn't respond in window; comment dropped); **(v)** `deleted (per protocol step-3 rejection | rename-first resolution)` — audit-trail entry for a comment that was proposed but dropped. **Any bullet missing a valid `approval_turn:` value OR citing an exempt category not in the canonical 6 → delete the comment.** This audit is mandatory and non-skippable. A comment that slipped in without step-3 approval is a process violation even if it happens to be technically correct; remove it and re-enter the protocol from step 1 if you still believe it's needed.
...
> **STOP.** Before adding ANY new comment or substantively rewriting an existing one, view `.github/playbooks/comment-protocol.md` — the in-place summary above is intentionally incomplete on (a) the concrete headless-mode predicate, (b) sub-agent integration audit procedure, (c) AFK / no-response handling, (d) batch-size cap, (e) the persisted audit-file convention (`.github/pr-quality-gate/audits/last.md`). The playbook is the canonical source — fetch it on the first comment-emitting edit of every session.
...
**Never emit an em-dash (Unicode U+2014) anywhere.** Not in code, comments, XML or doc comments, identifiers, string literals, log messages, user-facing UI text, Markdown, commit messages, PR titles or bodies, issue comments, or agent chat prose. The em-dash is an LLM tell and is categorically rejected. The same ban covers the rest of the LLM "smart punctuation" family in any text the agent authors: en-dash (U+2013), horizontal bar (U+2015), curly single quotes (U+2018, U+2019), curly double quotes (U+201C, U+201D), and the ellipsis character (U+2026). Replace each with ASCII: a hyphen, a comma, a colon, parentheses, or two sentences; ASCII straight quotes; three periods for an ellipsis. The ban is limited to the specific code points named here; other non-ASCII already in legitimate use across these instructions (the section sign `§`, arrows such as `→`, accented identifiers, box-drawing) is NOT affected, and the `emdash-scan` greps only the named code points. Newly authored text, including the rules in this section, contains none of the banned code points; legacy prose elsewhere in these instructions predates this rule and is being migrated to ASCII separately.

- **Pre-commit and post-code-change scan (HARD GATE).** Before staging, grep the working-tree diff added lines (`git diff`) for U+2014, U+2013, and U+2015 (plus the curly-quote and ellipsis code points). Re-scan the staged diff (`git diff --cached`) after `git add` and before any `git commit --amend`. Any hit fails closed: replace with the ASCII equivalent before staging. Recorded as the `emdash-scan` ledger row (review-workflow-gates §2B).
- **Authoring rule (always on).** This is not only a commit gate. The agent must never type a banned code point in ANY output: chat replies, plans, artifacts, `ask_user` prompts, sub-agent prompts. Treat those keys as nonexistent.
- **Exemptions.** Only genuinely vendored, generated, or third-party files that the change does not author are exempt (same precedence as the comment-protocol exempt categories). Never author a NEW banned code point to match surrounding vendored style.
...
- **Recorded in pre-PR-push phase state.** `pushCredentialsVerified` is a **required predicate field** (10th field of the state predicate — see *Per-phase additional fields*). Values: `yes` / `user-confirmed-unverifiable` / `blocked`. A `blocked` value fails the readiness gate. Sandbox-exit records also record this field with a real value (no `n/a-sandbox-exit` sentinel — §4.2 applies to sandbox pushes too). `preCreationReviewStatus` is the 11th predicate field, written by §2D per `pre-pr-creation-review.md`.
...
| `cpp.instructions.md` | `**/*.cpp`, `**/*.h`, `**/*.hpp`, `**/*.cc`, `**/*.cxx`, `**/*.c` | C++ naming, formatting, member ordering, defensive COM patterns (IFACEMETHODIMP `[out]` null-checks + sentinel init, `SHStrDupW`-style zero-init, `noexcept` boundary), vcxproj configuration (`$(DefaultPlatformToolset)`, bare `10.0` Windows SDK, cross-platform sibling sweep) |
| `msbuild.instructions.md` | `**/*.csproj`, `**/*.targets`, `**/*.props`, `**/*.vcxproj`, `**/*.vcxproj.filters`, `**/Directory.Build.props`, `**/Directory.Build.targets`, `**/Directory.Packages.props` | MSBuild property-function string-literal escaping (`'\\'` for literal backslash), `<Exec ConsoleToMSBuild>` trailing-newline trim, locked-down env tool-acquisition pattern (PATH discovery first, fallback download last), `$(RestoreConfigFile)` honoring for internal-mirror NuGet feeds |
