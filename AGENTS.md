...
- **Scope reduction requires explicit user sign-off.** When a panel, audit, or review identifies work items and the agent or sub-agents recommend deferring, dropping, or descoping any item — regardless of rationale (YAGNI, low priority, compliance theater, borderline finding, etc.) — the recommendation MUST be presented to the user via `ask_user` with the item, the rationale for deferral, and the dissenting opinions (if any). The agent does NOT have authority to unilaterally drop scope. Only the user can decide what is deferred vs. what is done. This applies to: panel findings, audit remediation items, PR scope decisions, TODO list triage, and any other context where identified work is being deprioritized.
