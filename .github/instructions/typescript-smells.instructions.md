...
### React / hooks
- **Missing or incorrect `useEffect` dependency array - audit every effect.** An effect that reads props/state but omits them from the deps array captures a stale closure; an empty-or-absent array on a body that reads reactive values re-runs unexpectedly (sometimes infinitely). Spot: for every `useEffect`/`useMemo`/`useCallback`, confirm the deps array lists exactly the reactive values the body reads.
