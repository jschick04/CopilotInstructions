...
- **`aria-label` that is wrong, duplicated, or describes appearance not behavior.** A new interactive control's `aria-label` must read naturally and describe what the control DOES. Watch for duplicated words (`"Filter filter sets by tag"`), appearance-only labels (`"X icon"` instead of `"Close dialog"`), and missing labels on a new `<button>` / `<input>` / custom select. Spot new `aria-label=` / `AriaLabel=` on added controls and read each aloud. Acceptable: visible text already labels the control (no redundant aria-label overriding it).
