...
| `status` | enum `pending \| catalog-updated \| catalog-rejected \| superseded \| catalog-strengthened \| catalog-new \| catalog-ext \| catalog-existing \| catalog-validated` | Current state of the proposed rule. **Semantic definitions:** `pending` = proposed, not yet acted on; `catalog-updated` = slug promoted to catalog OR existing catalog rule updated; `catalog-rejected` = proposed slug was rejected; `superseded` = proposed slug replaced by a different mechanism; `catalog-strengthened` = existing catalog rule had its wording / audit-method widened (heavier than `catalog-ext`); `catalog-new` = slug is NEW in catalog (first-add event; subset of catalog-updated semantics tracking the introduction specifically); `catalog-ext` = existing slug extended with new wording / sub-case (lighter than catalog-strengthened - no audit-method change); `catalog-existing` = confirmation that slug was already in catalog at finding time (row is evidence, no catalog change needed); `catalog-validated` = process-confirmation row (HARD GATE caught a real issue, documents successful prevention rather than a miss). For a `process-violation` row: `pending` while the gap is open, then the appropriate `catalog-*` status (`catalog-updated`, or `catalog-strengthened` / `catalog-new` when an existing rule was widened / a new rule added) once the gap is mechanized into a gate/instruction. |
| `prior_acks_present` | text (slug list, comma-separated within field - RFC 4180 quoted when the value contains commas/quotes/newlines) | Slugs the agent acknowledged in `core_rules_acknowledged` at the time of the miss; empty if no ack; `none` if ack was absent; or a bare ack-presence token `yes` / `no` / `partial` for legacy finding-time rows where exact slugs were not captured (an explanatory note, when one exists, is carried in `divergence_override_history`; many token rows have none). Detects ack-gate effectiveness - was the slug acked but missed, or was the slug never acked? |
| `rule_in_base_instructions` | enum `true \| false` | Whether the proposed rule was already present in `AGENTS.md` / `.github/instructions/*.instructions.md` at the time of the miss. Distinguishes load-failure (rule absent) from application-failure (rule loaded but ignored). Empty = unknown (the value was not captured for the row); immutable like all finding-time fields, so blanks are left as-is rather than backfilled |
| `divergence_override_history` | text (RFC 4180 quoted) | If the gate's count-divergence WARN was overridden by the reviewer with `divergence_acknowledged: <reason>`, the reason text is logged here for audit. Also carries any explanatory note migrated off a legacy `prior_acks_present` ack-presence token (`yes` / `no` / `partial`) when that row predates the slug-list convention; not every token row has such a note, so this field is often empty |
...
- **Compliance-correction carve-out (pre-merge only):** an immutable finding-time value that violated a load-bearing invariant at introduction (limited to: deidentification per Project agnosticism, or RFC 4180 quoting) MAY be corrected in-place while its row is still unmerged, provided the fix touches only the offending tokens (stripping deidentification leakage, or normalizing quoting) and preserves the finding semantics and the `catalog_revision` pin. This is distinct from (and does not license) back-dating evolved scope onto a finding-time pin, which stays forbidden: the row was non-compliant at birth, not rewritten after the catalog moved.
