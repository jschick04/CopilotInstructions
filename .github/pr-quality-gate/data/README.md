...
| `divergence_override_history` | text (RFC 4180 quoted) | If the gate's count-divergence WARN was overridden by the reviewer with `divergence_acknowledged: <reason>`, the reason text is logged here for audit. Also carries the explanatory note migrated off a legacy `prior_acks_present` ack-presence token (`yes` / `no` / `partial`) when that row predates the slug-list convention |
