...
<!-- read-receipt-token: 5b0ec618 -->

## Purpose

Classify the working tree (user stages = review), confirm the staged set + message at commit-approval, confirm who commits, and record per the project's commit-message rules. Fires after `post-code-change.md` clears (build + tests + reviewer consensus + verify-fix green).
...
- **Comment audit on staged diff (HARD GATE per `comment-protocol.md` §3.1 + §Persisted audit record)** - every commit runs the §3.1 discipline on every NEW or substantively-rewritten comment in the staged diff (clarity-check -> rename-check -> step-3 `ask_user` OR a canonical-6 exempt category), AFTER the user stages the code, BEFORE `git commit`. Receipt vs inline tracking by repo, the user-authored-comment narrow-`ask_user` rule, and zero-count handling: the `comment_audit` field requirement below + `comment-protocol.md` (canonical) + `post-code-change.md` §2.6.
- **`PRE-COMMIT GATE PASSED` block emitted in the current turn** before any `git commit` tool call - §1B refuses `git commit` without this block (mirrors §1A's `DESIGN PANEL CONVERGED` enforcement at the implementation boundary and §2B's `POST-CODE-CHANGE LEDGER` enforcement at the staging boundary). Emit this block + `CODE-REVIEW PANEL CONVERGED` + `POST-CODE-CHANGE LEDGER` IN FULL above the `git commit` call in the same response (the commit call is LAST); a `see <block> this turn` forward-reference does NOT satisfy §1B - the block must already exist, not be promised.
- **A panel verdict token (`DESIGN_READY`, `CODE_REVIEW_READY`, etc.) does NOT satisfy this gate on project (non-instruction) repos.** Panel review is technical; this gate is user review - both independent, both must pass. Chaining a panel token into a commit / `git push` / `gh pr create` without an intervening `ask_user` commit-approval is a process violation. Full rule in `.github/pr-quality-gate/panel-policy.md` §"User commit-approval after panel CODE_REVIEW_READY".
...
Before any `git commit` (including `git commit --amend`, `git cherry-pick`, `git rebase`-driven commit replay, or any other tool call that produces a new commit object), the agent MUST emit a literal block in the **current turn** that records the outcome of each pre-commit gate. The block must appear in the same turn as the commit tool call. Per §1B, the absence of the block forbids the tool call - no rationalization (e.g., "the diff was approved a few turns ago", "the message is obvious from context", "this is just a small amend") is acceptable. In THIS instructions repo, also write the block to the gitignored receipt `.github/pr-quality-gate/audits/pre-commit-gate-last.md` (`parent_sha:` = `git rev-parse HEAD`, or `HEAD^` on `--amend`; plus a `commit_subject:` = proposed subject header) BEFORE committing; `scripts/check-pre-commit-gate.ps1` (pre-commit hook + the pre-push `copilot-audit-precommit` note gate, adoption-gated) refuses the commit without a fresh receipt. NEVER stage it - the post-commit hook flushes it to a note. Non-adopted consuming repos keep the block chat-only (no receipt).
...
- **`format_check`** - five boolean sub-fields. Any `no` on `single_line` / `subject_length_chars > 72` / etc. that contradicts the playbook's format rules MUST cause the agent to revise before re-emitting. Genuine `git revert`: single_line:no+body:yes+len>72 accepted.
...
- **`core_rules_acknowledged`** - REQUIRED enumeration of every HIGH-tier review-pass-only catalog slug. Schema and verification semantics are canonical in `panel-policy.md` §Per-rule acknowledgement; chat emits the caveman one-line-per-slug form (`post-code-change.md` §"core_rules_acknowledged - chat-emission form (caveman)"). Per-site citations are MANDATORY for `status:applied`; aggregate counts alone are INVALID. **Forcing-function recipe** for the most common HIGH-tier slugs:
    - `comment-necessity`: cite per-bullet `approval_turn:` from the §2.6 ledger (the `.github/pr-quality-gate/audits/last.md` receipt in this instructions repo, or the inline `comment_audit` block on consuming repos). The valid bullet forms + the canonical-6 exempt categories are canonical in `comment-protocol.md` §"Recording - post-code-change ledger interaction"; any other value = violation.
...
A block claiming a gate passed (e.g., `diff_approved: yes (turn 42)`) when the conversation record shows it did not is MORE severe than omitting the block. Self-report falsified entries proactively in the next turn + propose remediation (typically: revert the commit, restore working tree, re-run the gates).

### Skip conditions (none apply unless explicitly documented this session)

- The user stated in THIS session "skip the pre-commit block" (or an unambiguous equivalent waiver) for a specific commit or range - recorded in session state with the affected commit identifier.
- `git commit --amend --no-edit` mechanically restoring a previously-emitted committer timestamp without changing tree or message (e.g., after a successful `git rebase --continue`) - the original commit's `PRE-COMMIT GATE PASSED` block is preserved by reference.
