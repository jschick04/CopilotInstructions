...
Procedure runs ≥3 reviewers in parallel across different model families, requires each to emit a `VERDICT: <DESIGN_READY | NEEDS_ANOTHER_ROUND>` line at output end, and iterates until convergence (or max-loop escalation). Sub-agents are sandboxed: tooling discipline forbids `ask_user`, file modifications, and recursive sub-agent launches.
...
**Recording convergence in the LEDGER (this instructions repo).** When this panel backs a `post-code-change-panel: ran, unanimous` row, record each reviewer as one line of the `panel-transcript:` block in the `POST-CODE-CHANGE LEDGER` (`review-workflow-gates-sweeps.md` §2B): `- slot:<id> model:<id> family:<claude|gpt|gemini> role:<rubber-duck|code-review> tier:<heavy|light> verdict:<CODE_REVIEW_READY|NEEDS_REWORK> rounds:<n>`, plus exactly one `- findings:<one-line>` line per block recording what the panel caught and how it was resolved (a disclosure field, not proof; whole-value `<...>` placeholder fails closed). The transcript `verdict:CODE_REVIEW_READY` corresponds to a reviewer's final `VERDICT: DESIGN_READY` (and `NEEDS_REWORK` to `NEEDS_ANOTHER_ROUND`); `rounds` is how many rounds that reviewer took to converge. `Test-PanelLedger` validates the slate against the full floor (always full in this repo, per `panel-policy.md` §"Mechanical enforcement"); a `slot` is the unique reviewer identity (distinct-by-slot). This is honest-ceiling: it records the converged composition, not proof the sub-agents ran.
