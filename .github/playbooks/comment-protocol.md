...
- **`deletion`** - removing a comment entirely (non-purge, edit-time removal). Governed by AGENTS.md §3.1 *Remove existing stale comments* rule. Step-2.4 comment-hygiene-purge removals are NOT recorded here - they are counted on the `comment-hygiene-purge` ledger row (see `comment-hygiene-purge.md`).
- **`stale-comment-fix-per-§3.9/§3.10`** - correcting a comment that has become false because of your code change. §3.9 / §3.10 REQUIRE these fixes. Apply rename-first / delete-default; do NOT route through step 3. Recorded on the ledger with this exact category label. (A step-2.4 purge removal attributes to the purge row, staleness aside.)
