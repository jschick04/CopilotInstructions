...
- **Pattern-catalog files are path-excluded from detection (deliberate policy).** `Test-IsNewCommentLine` returns `$false` for `pattern-catalog.md` and the markdown under `pattern-catalog.sources/`: those files enumerate comment-syntax tokens as rule examples a flat regex cannot tell from real comments. Total: ANY real comment there is NOT audited. Accepted because the catalog is human-reviewed governance content (that review is the backstop). A non-markdown file under `pattern-catalog.sources/` is still audited.
