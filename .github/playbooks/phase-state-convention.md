...
- `phase-state-implementation-<ts>` - the IMPLEMENTATION node, written at the `IMPLEMENTATION CHECKPOINT` (`post-code-change.md` §2.8). Carries `design_ready_ref: phase-state-pre-implementation-<ts>` and `diff_matches_design: <yes | diverged:"...">`; its own `status` column is `in_progress` then `done` on completion (the checkpoint's `status: complete` lives in the LEDGER + chat block, NOT duplicated here - a second `status:` line would collide with the reader-contract `status:` field below).
