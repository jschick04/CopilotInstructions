...
3. **Helper / util / manager anti-pattern** — `grep` for class names ending in `Helper` / `Util` / `Utils` / `Manager`. Propose renaming to what the code does (`DatabasePathSorter` not `DatabaseHelpers`).
