...
**Recurrence pattern**: Copilot's pattern matcher appears to treat the raw-string delimiter syntax as literal characters, especially when the delimiter is a single non-alphanumeric (`-`, `=`, `*`) or when the content itself contains `%s` format specifiers (which may confuse the matcher's heuristics). Tends to recur in clusters when one file uses several raw-string literals with the same delimiter (the matcher applies the mis-parse to each). **Empirical confirmation (PR-563)**: same 3 literals flagged on EventLogExpert PR #563 R3 (3344554105/-140/-167), then RE-flagged on the same lines on R8 (3344982929/-953/-976) after unrelated amends. The same Copilot model version did not "remember" the previous flag — each fresh review regenerates the misparse. Cross-amend recurrence is the rule, not the exception.
...
- **Switch from raw-string to escaped-string** (`L"\"%s\""` instead of `LR"-("%s")-"`). Loses some raw-string readability when the content has multiple embedded quotes / backslashes, but for short format strings (≤ 1 embedded quote pair, no embedded backslashes) the escaped form is no less readable than the raw-string form — and for format strings with NO special characters (e.g., `L"%s %s"` instead of `LR"-(%s %s)-"`), the raw-string was actually over-engineered. **Apply this mitigation when the same file triggers the FP repeatedly across model updates** (≥ 2 amend cycles). **Empirically validated on PR-563 R8**: switched 3 literals (`LR"-("%s")-"` → `L"\"%s\""`, two instances of `LR"-(%s %s)-"` → `L"%s %s"`) after the FP recurred on the same lines that R3 had already flagged. Net effect: zero behavioral change, eliminated the recurring FP class, and made two of the three literals strictly simpler (no escapes needed at all). The "switch to escaped" mitigation is the right answer when (a) the content is short, (b) the embedded-special-char count is ≤ 2, and (c) the FP has recurred across amends.
...
- **Default status**: keep raw-string when the content has many embedded quotes / backslashes that escapes would obscure (regex patterns, JSON literals, multi-line strings). Switch to escaped when the FP recurs on short / quote-light strings (the PR-563 R8 pattern).
