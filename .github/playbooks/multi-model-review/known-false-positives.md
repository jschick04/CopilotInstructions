...
**Recurrence pattern**: Copilot's pattern matcher appears to treat the raw-string delimiter syntax as literal characters, especially when the delimiter is a single non-alphanumeric (`-`, `=`, `*`) or when the content itself contains `%s` format specifiers (which may confuse the matcher's heuristics). Tends to recur in clusters when one file uses several raw-string literals with the same delimiter (the matcher applies the mis-parse to each). **Empirical confirmation**: the same 3 literals were flagged on one PR's round 3, then RE-flagged on the same lines on a later round after unrelated amends. The same Copilot model version did not "remember" the previous flag - each fresh review regenerates the misparse. Cross-amend recurrence is the rule, not the exception.
...
- **Default status**: keep raw-string when the content has many embedded quotes / backslashes that escapes would obscure (regex patterns, JSON literals, multi-line strings). Switch to escaped when the FP recurs on short / quote-light strings (the recurring-FP pattern).
