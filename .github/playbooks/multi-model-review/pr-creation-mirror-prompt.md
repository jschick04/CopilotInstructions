...
Categories are the substantive contract; consumer-specific config (slate, convergence, output format) lives in the consumer's playbook.
...
Stay within the consumer prompt's stated scope. A clean verdict is a clean
verdict; do not pad with adjacent observations.
...
Complements cross-file sweep and self-similarity sweep - fills the gap where
remediation patterns from earlier commits were never re-engaged in later panel
runs. Optimization: always run if prior round flagged a missing-sister-site;
otherwise a one-time re-check skippable on subsequent panels with no new patterns.
...
Distinct from cross-file sweep and self-similarity sweep - catches the
failure mode where the verification argument considers only ONE interleaving.
...
**Rationale review (judgment surface)**: `rationale:` for `status:
not-applicable` cannot be mechanically falsified - it is judgment text.
The §2D heavy-slate panel reviews these; thin rationales ("no risk",
"doesn't apply") are panel-rejectable as blocking findings per §2B.
Per-commit panels flag suspicious rationales but do NOT block (§2D is the
substantive review).

**Examples of remediation patterns P** (illustrative; applies regardless of
language/framework): resource-cleanup symmetry, bounds-clamp before bulk copy,
null-coalesce in display strings, exception-catch around interop calls in
component lifecycle, pool-flush before file-handle operations.
