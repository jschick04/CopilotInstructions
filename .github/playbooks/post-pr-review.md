...
**Panel scope = the whole branch, re-attested on every review-response push.** The panel reviews the FULL whole-branch diff `panelBaseSha..HEAD` (per `pre-pr-creation-review.md` Step 2 default-full policy), NOT just the review-response fix diff. On every review-response push - the PR already exists, so this is NOT a "pre-PR" op - re-run the `pre-pr-creation-review.md` whole-branch panel (Steps 2-3) AND re-emit its Step 7 `QUALITY GATE` block with the `panel_coverage` field, so whole-branch coverage is provably re-attested for THIS push. Required surface = those two review-coverage steps ONLY; do NOT re-run the PR-creation-only tool step (G6 / Step 10 `gh pr create`), which fires once at creation. The §6 `PR review audit` block below is findings-only (no base..head / coverage scope) and does NOT satisfy that whole-branch panel-coverage requirement. Carry-forward of prior dispositions is allowed only as the explicit user-authorized exception recorded in `panel_coverage` (default is a full re-read).
...
### 4.5. RCA the miss + route it (repo-aware)

A bot/human finding that NONE of our prior LOCAL scans caught - the pre-PR publish-gate panel, the deterministic gates, or self-review - is a panel/gate MISS, not just a code fix. For each such finding - status `fixed`, `routed-now`, OR `routed-deferred` (exclude ONLY `dismissed-source-grounded`, the bot-was-wrong case) - that is generalizable beyond this one site:

1. **RCA (forcing function).** Name the SPECIFIC existing-or-derivable coverage that should have surfaced it - a `pattern-catalog` slug (and its `discovery_query`), a deterministic gate, or the publish-gate panel prompt - and WHY it slipped (slug gap, missing slug, panel anti-anchoring miss, scope-not-whole-branch, etc.). If NO such gate / slug / prompt can be named, the finding is a genuine one-off (typo / taste) and is EXEMPT - no RCA routing.
2. **ask_user - route by which repo the PR is in** (NEVER auto-act). Present the one-line RCA, then:
   - **PR in a CONSUMING project** (the PR repo is NOT the instruction-set repo where these playbooks live): choose **decline to file** OR **file a tracking issue in the instruction-set repo** - the process gap is encoded in the governance repo later, not in the consuming project's PR. If the user declines to file (or does not respond), do NOT file.
   - **PR in the instruction-set repo itself** (resolve via the clone's `origin`; this IS the governance repo): choose **fix now** (fold the gate / slug / prompt fix into THIS PR or a sibling PR) OR **add as a follow-up task** recorded as a §6 external record (a committed task-file entry / session-note path / sibling-PR ref - NOT a bare session todo, which can be skipped under the same momentum that caused the miss; prefer **fix now**). Do NOT open a GitHub issue against the repo you are already editing - filing an issue to yourself when you can fix it is busywork; fix it or queue it as a durable task.
3. **On "file a tracking issue" (consuming-project route only).** `gh issue create --repo <instruction-set-repo> --title "<generic miss-class>" --body "<RCA + proposed fix>"` - resolve `<instruction-set-repo>` from the instruction-set clone's `origin` remote (where these playbooks live), NOT the current project repo: the issue MUST land in the governance repo, never the consuming project. Use a GENERIC, no-project-specifics title / body per §5 (the same project-agnostic rule that governs instruction deltas) - describe the CLASS of miss and the shape of the gate / slug fix, never a real symbol / path / PR number. Suggested label: `panel-miss`.
4. **Always** record the miss in the instruction-set repo's `.github/pr-quality-gate/data/panel-misses.csv` (the governance repo, NOT the consuming project) regardless of the route - the CSV is the local anti-recidivism ledger; the durable tracker is the issue (consuming-project route), the in-PR fix / sibling-PR (instruction-set route), OR - for a DEFERRED instruction-set task - a §6 external record (committed task-file / session-note path; a bare session todo is a bypassable recall aid, NOT a durable tracker).

**Evidence-gate interaction (§6):** the process-gap routing is a DISTINCT meta-entry, NOT a change to the original finding's own disposition. A `fixed` finding stays `fixed`; the miss is logged as a separate `routed-deferred` meta-row whose external record is the issue URL (consuming-project route), the sibling-PR ref OR a §6 external record for a deferred task (instruction-set route - a committed task-file / session-note path, NOT a bare session todo), OR - when the user declines to file / does not respond - the `panel-misses.csv` entry (file path + the appended row). All satisfy §6's "no deferral without an external record". Do not downgrade the original finding's status because a process-gap meta-row was logged.
...
> **The when-to-re-run-sweep rules below govern the mechanical branch-wide SWEEP (rename / least-privilege / VSA) ONLY - NOT the publish-gate multi-model panel.** The multi-model panel's SCOPE is the WHOLE branch on every review-response push (default full re-read; carry-forward only as the gate-authorized `panel_coverage` exception), independent of the sweep decision (per §3 above + `pre-pr-creation-review.md`); a "per-commit audit only" sweep outcome never bounds the panel's whole-branch scope.
