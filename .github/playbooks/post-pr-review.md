...
2. **ask_user (opt-in-to-file).** Present the one-line RCA and propose filing a tracking issue in the instruction-set repo. The user approves or declines. On decline OR no response, do NOT file - log to `panel-misses.csv` only (the anti-recidivism ledger; see step 4 below). NEVER auto-file.
