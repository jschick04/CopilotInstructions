...
2. **ask_user - route by which repo the PR is in** (NEVER auto-act). Present the one-line RCA, then:
   - **PR in a CONSUMING project** (the PR repo is NOT the instruction-set repo where these playbooks live): choose **dismiss** OR **file a tracking issue in the instruction-set repo** - the process gap is encoded in the governance repo later, not in the consuming project's PR. On dismiss / no response, do NOT file.
