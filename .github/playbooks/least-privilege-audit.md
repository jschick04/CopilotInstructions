...
- **IVT scope must be a deliberate, recorded decision (invariant).** `InternalsVisibleTo` (and equivalent friend-grants) default to TEST / test-util targets. A PRODUCTION cross-module consumer reaching across the friend boundary is allowed ONLY as a deliberate, recorded choice that justifies why a real `public` API (via the DI-seam ladder above) or a same-solution app-composition seam is unsuitable - never a reflex "it compiles / by convention." Audited via the POST-CODE-CHANGE LEDGER `touched-file-LPA` field, which OWNS IVT-scope; the `dependency-injection-fit` field records DI-shape only and CITES this for IVT evidence, never re-decides it (never record the same constraint in both).
