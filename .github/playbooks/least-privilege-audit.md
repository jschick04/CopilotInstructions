...
- **OTHER-asm prod consumers** — BLOCK internalization (the type is genuinely cross-asm public).
- **OTHER-asm test consumers** without friend grant — DECISION POINT: recommend either *add IVT to that test asm* OR *keep public*. Surface to user.

Recommendation values: `INTERNAL` / `INTERNAL+ADD-FRIEND-GRANT` / `KEEP-PUBLIC`.
