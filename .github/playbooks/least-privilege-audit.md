...
- **Friend-grant proliferation is a coupling cost** - adding a NEW friend-grant exposes ALL current AND future internals of the granting asm to the receiving asm. Precedence ladder in `axis-1-type-access.md`: (1) split-member visibility, (2) co-location, (3) DI-seam extraction (public interface + DI registration extension + internal sealed impl), (4) keep public, (5) add new friend-grant (LAST resort). Re-using an existing friend-grant is free; adding a new one is the LAST resort.
- **IVT scope must be a deliberate, recorded decision (invariant).** `InternalsVisibleTo` (and equivalent friend-grants) default to TEST / test-util targets. A PRODUCTION (non-test) cross-module consumer reaching across the friend boundary is allowed ONLY as a deliberate, recorded choice that justifies why a real `public` API (via the DI-seam ladder above) or a same-solution app-composition seam is unsuitable - never a reflex "it compiles / by convention." A **test target** (canonical definition) is an assembly whose name carries a test / test-double convention as a PascalCase segment: `Test`/`Tests` (incl. `Unit`/`Integration`/`Functional`/`Acceptance`/`TestUtil` forms), `Spec`/`Specs`, `Fake`/`Fakes`, `Mock`/`Mocks`, `Fixture`/`Fixtures`, `Benchmark`/`Benchmarks`, `Stub`/`Stubs`, `E2E` - a conservative name-proxy for the semantic "no production consumer" test (a genuinely test-only project named otherwise is the deliberate-decision case). Audited via the POST-CODE-CHANGE LEDGER `touched-file-LPA` field, which OWNS IVT-scope: for a NON-test target the field MUST carry the `production-ivt:` marker INSIDE the standard count shape - `ran (N findings, K unjustified; production-ivt: <why a test target / DI-seam / public API is unsuitable>)` - a single value that satisfies BOTH the count shape required by `least-privilege-audit-required-on-visibility-delta` AND the marker required here (a bare `ran` / `N/A`, or a count shape with no `production-ivt:` marker, is rejected). The B1 floor (`scripts/lib/hygiene-signals.psm1`) mechanically FLAGS a line-start added non-test IVT (a minified-inline / commented / string-literal grant is a disclosed false-negative) and FORCES the `production-ivt:` marker present (shape-checked, not aptness-verified); the `non-test-ivt-target-without-recorded-decision` catalog slug + the panel verify the justification is apt. The `dependency-injection-fit` field records DI-shape only and CITES this for IVT evidence, never re-decides it (never record the same constraint in both).
...
- **Catalog rule cross-reference**: this playbook's "playbook ran when visibility delta in diff" invariant is enforced by catalog rule `least-privilege-audit-required-on-visibility-delta` (HIGH, review-pass-only). The rule fires when a commit's diff contains a visibility delta AND the POST-CODE-CHANGE LEDGER `touched-file-LPA` field is absent or has a bare `N/A` value without a documented carve-out citation. This catches the failure mode where the agent recognizes the diff visually but forgets to invoke the playbook. See `.github/pr-quality-gate/pattern-catalog.md` row `least-privilege-audit-required-on-visibility-delta` for the full audit method.
...
- *"Add a new friend-grant so we can internalize this one type."* REJECT when split-member visibility, co-location, **DI-seam extraction**, or KEEP-PUBLIC would satisfy the cross-asm consumer's needs without proliferating IVT / module-export coupling. See `axis-1-type-access.md` precedence ladder.
