...
- **Reconcile-before-acting** - when a user decision returns (an `ask_user` answer or a go-ahead), the FIRST move is to restate the gates / commitments owed for the scope and satisfy them BEFORE any edit; do not carry the go-ahead's momentum straight into worktree edits.
- **No unilateral gate carve-outs; before-implementing = before ANY edit** - "part of this scope is exempt from the committed gate / can go first" is an `ask_user`, never an agent decision. Reading / investigating is fine pre-gate; the gate blocks editing, and it blocks ALL of the scope's edits, not a self-chosen subset. (Discipline aids, not mechanical teeth - no script reads a verbal commitment or blocks the momentum edit; the `committed-gate-skipped-or-unilaterally-carved-out` slug is the commit-time DETECTIVE backstop.)
...
**What "unanimous `DESIGN_READY`" actually requires (the gate exit condition).** Only an UNCONDITIONAL `DESIGN_READY` with zero open changes clears the gate. A verdict that is conditional or carries unresolved / enumerated changes - "approve with changes", "looks good but fix X", "`DESIGN_READY` pending Y" (or any conditional shape a non-native panel emits, e.g. "GO-WITH-CHANGES") - is structurally a `NEEDS_REWORK`, NOT convergence: a positive-sounding verdict is not a terminal one. Incorporate the changes, then re-run the panel to unanimous, unconditional `DESIGN_READY` before ANY production edit (a "production edit" is any `create` / `edit` / file-write / `git add` / impl sub-agent on the code or governance artifact under change (the same boundary the PRE-EDIT SENTINEL fires on - and governance/instruction artifacts are never trivial); reading / investigating stays fine, and per the before-implementing rule above the gate blocks all of the scope's edits, not just the first).
