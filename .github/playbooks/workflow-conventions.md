...
For any session that will make code or governance changes, the agent MAY register a recurring self-audit schedule (~4 min) that runs `git status --porcelain`. On a hit (edits AND the phase-chain is broken for the current change - no live-transcript `DESIGN PANEL CONVERGED` / `PRE-EDIT SENTINEL`, OR an `IMPLEMENTATION CHECKPOINT` whose `design_ready_ref` predecessor is not readable in durable phase-state - where a block present only in a summary counts as absent), STOP and RECONSTRUCT from durable phase-state (`phase-state-convention.md` chain) / §2B evidence or re-run the missing gate before any further edits or commit. Re-arm this schedule whenever such a session resumes from a summary, independent of current tree state (the next-commit case starts with a clean tree). Prompt template:

> SELF-AUDIT phase-chain tripwire. Run `git -C <repo> status --porcelain`. If it shows any modified/added/staged file AND the phase-chain for that change is broken - no real `DESIGN PANEL CONVERGED` / `PRE-EDIT SENTINEL` in your live transcript, OR an `IMPLEMENTATION CHECKPOINT` whose `design_ready_ref` predecessor is not readable in durable phase-state (a block present only in a summary counts as absent): STOP, tell the user the design->impl proof is missing (possibly lost to summarization, not necessarily skipped), and RECONSTRUCT from durable phase-state / §2B evidence or re-run the missing gate before any further edits or commit. If the chain IS on record, reply `self-audit: chain on record` and continue. Never take destructive action. Stop the schedule once the change ships.
