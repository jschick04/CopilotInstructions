...
For any session that will make code or governance changes, the agent MAY register a recurring self-audit schedule (~4 min) that runs `git status --porcelain`. On a hit (edits AND no live-transcript `PANEL CONVERGED` / `PRE-EDIT SENTINEL` block for the current change, where a block present only in a summary counts as absent), STOP and re-establish it from durable §2B evidence or re-run the panel before any further edits or commit. Re-arm this schedule whenever such a session resumes from a summary, independent of current tree state (the next-commit case starts with a clean tree). Prompt template:
