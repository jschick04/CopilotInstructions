...
- **`nameof()` for code symbols inside diagnostic strings — mandatory.** Any string that embeds the name of a type, method, property, field, parameter, or local variable MUST use `nameof(...)` instead of a hardcoded literal. `nameof()` is a compile-time constant (zero runtime cost) and survives renames; hardcoded names silently rot when the symbol is renamed and the next reader sees a log that names a method that no longer exists. Common surfaces:
